...
BOMStorage.prototype.getHeader = function (size_of_vars, size_of_header, size_of_free_list) {
  ...
  var size_of_block_table = 8 + 4 + this.entries.length * 8 // BOMPointer (adress + length)
...
BOMStorage.prototype.getVars = function (size_of_vars) {
  ...
  this.vars.forEach(function (vari) {
    vars.writeUInt32BE(vari.index + 1, pos); pos += 4
...
BOMStorage.prototype.createReadStream = function () {
  ...
  return readable(function () {
    ...
    // block table
    var block_table_length = new Buffer(4)
    block_table_length.writeUInt32BE(self.entries.length + 1, 0)
    this.push(block_table_length)

    var nullEntry = new Buffer(8)
    nullEntry.fill(0)
    this.push(nullEntry)
    ...
    freeList.writeUInt32BE(0, 0) // ? entries

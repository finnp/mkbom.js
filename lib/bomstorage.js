...
BOMStorage.prototype.getHeader = function (size_of_vars, size_of_header, size_of_free_list) {
  ...
  header.writeUInt32LE(headerInfo.version, 2*4)
  header.writeUInt32LE(headerInfo.numberOfBlocks, 3*4)
  header.writeUInt32LE(headerInfo.indexOffset, 4*4)
  header.writeUInt32LE(headerInfo.indexLength, 5*4)
  header.writeUInt32LE(headerInfo.varsOffset, 6*4)
  header.writeUInt32LE(headerInfo.varsLength, 7*4)
...
BOMStorage.prototype.getVars = function (size_of_vars) {
  ...
  vars.writeUInt32LE(this.vars.length, 0)
  this.vars.forEach(function (vari) {
    vars.writeUInt32LE(vari.index, 0)
    vars.writeUInt8(vari.name.length, 4)
    vars.write(vari.name, 5)
  })
...
BOMStorage.prototype.createReadStream = function () {
  ...
  return readable(function () {
    ...
    self.entries.reduce(function (address, buf) {
      ...
      blockEntry.writeUInt32LE(address, 0)
      blockEntry.writeUInt32LE(buf.length, 4)
      this.push(blockEntry)
      return address
    }.bind(this), size_of_header + size_of_vars)
    // free list
    var freeList = new Buffer(size_of_free_list)
    freeList.writeUInt32LE(2, 0) // 2 entries
    freeList.writeUInt32LE(0, 1 * 4) // adress
    freeList.writeUInt32LE(0, 2 * 4) // length
    freeList.writeUInt32LE(0, 3 * 4) // adress
    freeList.writeUInt32LE(0, 4 * 4) // length 
    this.push(freeList)
  })
}
